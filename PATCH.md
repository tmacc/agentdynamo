...
- `Important invariants`:
  ...
  - Worktree forks must branch from the source conversation's current checkout branch, preferring live git status for the source workspace over stale/default thread metadata or UI fallbacks. The dialog label should show that same live base branch when available. Fork worktrees must also receive the source workspace's dirty tracked changes, deletions, and untracked non-ignored files as unstaged changes; ignored files stay excluded.
...
- `Verification`:
  ...
  - From a thread whose workspace is checked out on a non-default branch, create a worktree fork and confirm the new worktree is based on that branch rather than `main`.
