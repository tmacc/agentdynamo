      ...
      probe: {
        ...
        message:
          openCodeSettings.serverUrl.trim().length > 0
            ? "OpenCode is disabled in Dynamo settings. A server URL is configured."
            : "OpenCode is disabled in Dynamo settings.",
      },
    });
  }
    ...
    const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (input: {
      ...
      if (!isExternalServer) {
        ...
        if (!version) {
          return fallback(
            new Error(
              `Unable to determine OpenCode version from \`opencode --version\` output. Dynamo requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
            ),
            null,
          );
        }
