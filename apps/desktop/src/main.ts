...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  env[APP_HOME_ENV_VAR] = BASE_DIR;
  env[LEGACY_APP_HOME_ENV_VAR] = BASE_DIR;
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    return (
      normalizeCommitHash(parsed[APP_COMMIT_HASH_FIELD]) ??
      normalizeCommitHash(parsed.t3codeCommitHash)
    );
  } catch {
    return null;
  }
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Dynamo failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Dynamo ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses. This is unfriendly for shell usage and violates Linux naming
 * conventions.
 *
 * We override it to a clean lowercase Dynamo-specific name so upstream T3 Code and
 * Dynamo can run on the same machine without sharing Chromium profile data
 * (localStorage, cookies, sessions).
 */
