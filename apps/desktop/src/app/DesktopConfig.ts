...
const preferredTrimmedString = (primaryName: string, fallbackName: string) =>
  Config.all({
    primary: trimmedString(primaryName),
    fallback: trimmedString(fallbackName),
  }).pipe(Config.map(({ primary, fallback }) => (Option.isSome(primary) ? primary : fallback)));
