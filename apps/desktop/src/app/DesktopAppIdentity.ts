...
import { APP_COMMIT_HASH_FIELD } from "@t3tools/shared/branding";
...
const AppPackageMetadata = Schema.Struct({
  [APP_COMMIT_HASH_FIELD]: Schema.optional(Schema.String),
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed[APP_COMMIT_HASH_FIELD] ?? parsed.t3codeCommitHash).pipe(
