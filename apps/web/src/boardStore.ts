...
function newFeatureCardId(): FeatureCardId {
  return FeatureCardId.make(randomUUID());
}
...
interface BoardStoreState {
  ...
  readonly optimisticUpsertCard: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
    card: FeatureCard,
  ) => void;
  readonly optimisticMoveCard: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
  ...
  readonly optimisticArchiveCard: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
    cardId: FeatureCardId,
    archivedAt: string,
  ) => void;
  readonly optimisticDeleteCard: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
    cardId: FeatureCardId,
  ) => void;
  readonly optimisticDismissGhost: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
    threadId: ThreadId,
    dismissedAt: string,
  ) => void;
  readonly optimisticUndismissGhost: (
    environmentId: EnvironmentId,
    projectId: ProjectId,
    threadId: ThreadId,
  ) => void;
...
export const useBoardStore = create<BoardStoreState>((set) => ({
  ...
  optimisticUpsertCard: (environmentId, projectId, card) =>
    set((state) => {
      ...
      const filtered = existing.filter((candidate) => candidate.id !== card.id);
      return {
        cardsByKey: {
          ...state.cardsByKey,
          [key]: [...filtered, card],
  ...
  optimisticDeleteCard: (environmentId, projectId, cardId) =>
    set((state) => {
      ...
      return {
        cardsByKey: {
          ...state.cardsByKey,
          [key]: existing.filter((candidate) => candidate.id !== cardId),
        },
      };
    }),
  ...
  optimisticDismissGhost: (environmentId, projectId, threadId, dismissedAt) =>
    set((state) => {
      ...
      if (existing.some((ghost) => ghost.threadId === threadId)) {
        return state;
      }
      return {
        dismissedGhostsByKey: {
  ...
  optimisticUndismissGhost: (environmentId, projectId, threadId) =>
    set((state) => {
      ...
      return {
        dismissedGhostsByKey: {
          ...state.dismissedGhostsByKey,
          [key]: existing.filter((ghost) => ghost.threadId !== threadId),
        },
      };
    }),
}));
...
function applyBoardEventToState(
  ...
  switch (event.type) {
    case "board.card-created": {
      ...
      const nextCard: FeatureCard = {
        id: payload.cardId,
        projectId: payload.projectId,
        title: payload.title,
        description: payload.description,
        seededPrompt: payload.seededPrompt,
        column: payload.column,
        sortOrder: payload.sortOrder,
        linkedThreadId: payload.linkedThreadId,
        linkedProposedPlanId: payload.linkedProposedPlanId,
      ...
      return {
        cardsByKey: {
          ...state.cardsByKey,
          [key]: [...cards.filter((candidate) => candidate.id !== nextCard.id), nextCard],
        },
        snapshotSequenceByKey: { ...state.snapshotSequenceByKey, [key]: event.sequence },
      };
    }
            ...
            candidate.id === payload.cardId
              ? {
                  ...
                  ...(payload.description !== undefined
                    ? { description: payload.description }
                    : {}),
                  ...(payload.seededPrompt !== undefined
                    ? { seededPrompt: payload.seededPrompt }
                    : {}),
                  updatedAt: payload.updatedAt,
                }
    ...
    case "board.card-deleted": {
      ...
      return {
        cardsByKey: {
          ...state.cardsByKey,
          [key]: cards.filter((candidate) => candidate.id !== payload.cardId),
        },
        snapshotSequenceByKey: { ...state.snapshotSequenceByKey, [key]: event.sequence },
      };
    }
        ...
        cardsByKey: {
          ...
          [key]: cards.map((candidate) =>
            candidate.id === payload.cardId
              ? {
                  ...candidate,
                  linkedThreadId: payload.threadId,
                  updatedAt: payload.updatedAt,
                }
              : candidate,
          ),
    ...
    case "board.ghost-card-dismissed": {
      ...
      return {
        dismissedGhostsByKey: {
          ...state.dismissedGhostsByKey,
          [key]: dismissed.some((ghost) => ghost.threadId === payload.threadId)
            ? dismissed
            : [
                ...dismissed,
                {
    ...
    case "board.ghost-card-undismissed": {
      ...
      return {
        dismissedGhostsByKey: {
          ...state.dismissedGhostsByKey,
          [key]: dismissed.filter((ghost) => ghost.threadId !== payload.threadId),
        },
        snapshotSequenceByKey: { ...state.snapshotSequenceByKey, [key]: event.sequence },
      };
    }
...
export function useBoardCards(
  ...
  return useBoardStore((state) => selectCardsForProject(state, environmentId, projectId));
}
...
export function useBoardDismissedGhostThreadIds(
  ...
  return useBoardStore(
    useShallow((state) => {
      const dismissed = selectDismissedGhostsForProject(state, environmentId, projectId);
      return new Set(dismissed.map((item) => item.threadId));
    }),
  );
}

export function useBoardStatus(environmentId: EnvironmentId, projectId: ProjectId) {
  return useBoardStore(useShallow((state) => selectBoardStatus(state, environmentId, projectId)));
}
...
export function acquireBoardSubscription(
  ...
  entry.unsubscribeConnectionListener = subscribeEnvironmentConnections(() => {
    try {
      ...
      useBoardStore.getState().setStatus(environmentId, entry.projectId, "error", message);
    }
  });
...
export function computeNextSortOrderForAppend(
  ...
  const inColumn = sortByOrder(
    cards.filter((card) => card.column === column && card.archivedAt === null),
  );
...
export function computeSortOrderBetween(
  ...
  const before = beforeId ? inColumn.find((card) => card.id === beforeId) : null;
  const after = afterId ? inColumn.find((card) => card.id === afterId) : null;
  const previousSort = before ? before.sortOrder : null;
  const nextSort = after ? after.sortOrder : null;
  const sortOrder = computeMidpointSortOrder(previousSort, nextSort);
  const needsReindex =
    previousSort !== null &&
    nextSort !== null &&
    Math.abs(nextSort - previousSort) < BOARD_REINDEX_THRESHOLD * 4;
...
export async function updateBoardCard(input: {
  ...
  try {
    ...
    store.optimisticUpsertCard(input.environmentId, input.projectId, existing);
    throw error;
  }
}
