...
import { archiveBoardCard, deleteBoardCard, dismissGhostCard } from "../../boardStore";
import type { BoardItem } from "../../boardProjection";
...
export const BoardUserCard = memo(function BoardUserCard({
  ...
  return (
    <Card
      ref={dragHandleRef}
      style={style}
      className={cn("group", KANBAN_CARD_CLASSES, isDragging ? "opacity-0" : "hover:shadow-md")}
      {...attributes}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2 border-t px-3 py-1.5 text-xs text-muted-foreground">
          ...
          <BoardCardOverflowMenu
            onArchive={() => {
              void archiveBoardCard({ environmentId, projectId, cardId: card.id }).catch(
                () => undefined,
              );
            }}
            {...(card.linkedThreadId
              ...
              : {
                  onDelete: () => {
                    void deleteBoardCard({ environmentId, projectId, cardId: card.id }).catch(
                      () => undefined,
                    );
                  },
                })}
...
export const BoardLiveCard = memo(function BoardLiveCard({
  ...
  const { thread, linkedCard, isGhost } = item;
      ...
      {isGhost ? (
        <div className="flex items-center justify-end border-t px-3 py-1 text-xs text-muted-foreground">
          <Button
            ...
            onClick={(event) => {
              event.stopPropagation();
              void dismissGhostCard({ environmentId, projectId, threadId: thread.id }).catch(
                () => undefined,
              );
            }}
          >
