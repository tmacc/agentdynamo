...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  ...
  },
  {
    ...
    serverUrlDescription: "Leave blank to let Dynamo spawn the server when needed",
...
function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      headline: "Disabled",
      detail:
        provider.message ?? "This provider is installed but disabled for new sessions in Dynamo.",
    };
  }
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how Dynamo looks across the app."
          resetAction={
            theme !== "system" ? (
        ...
        <SettingsRow
          title="Team agents"
          description="Allow coordinator threads to spawn child agents across supported providers."
          resetAction={
            !Equal.equals(settings.teamAgents, DEFAULT_UNIFIED_SETTINGS.teamAgents) ? (
              <SettingResetButton
                label="team agents"
          ...
          control={
            <Switch
              checked={settings.teamAgents.enabled}
              onCheckedChange={(checked) =>
                updateSettings({
                  teamAgents: {
                    ...settings.teamAgents,
